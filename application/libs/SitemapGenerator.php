...
class SitemapGenerator
{
    ...
    public function addUrls($urlsArray)
    {
        ...
        foreach ($urlsArray as $url) {
            $this->addUrl(isset($url[0]) ? $url[0] : null, isset($url[1]) ? $url[1] : null, isset($url[2]) ? $url[2] : null, isset($url[3]) ? $url[3] : null, isset($url[4]) ? $url[4] : null);
        }
    }

    public function addUrl($url, $alternateLanguages = null, $lastModified = null, $changeFrequency = null, $priority = null)
    {
        if ($url == null)
        ...
        if (isset($alternateLanguages))
            $tmp['languages'] = $alternateLanguages;
    ...
    public function createSitemap()
    {
        ...
        $sitemapIndexHeader = '<?xml version="1.0" encoding="UTF-8"?>' . $generatorInfo . '<sitemapindex xmlns="http://www.sitemaps.org/schemas/sitemap/0.9"></sitemapindex>';
        foreach (array_chunk($this->urls, $this->maxURLsPerSitemap) as $sitemap) {
            ...
            foreach ($sitemap as $url) {
                ...
                if (isset($url['languages'])) {
                    $languages = explode(',', $url['languages']);
                    if (!empty($languages)) {
                        foreach ($languages as $language) {
                            $xhtml_link = $row->addChild('xhtml:link', null, 'http://www.w3.org/1999/xhtml');
                            $xhtml_link->addAttribute('rel', 'alternate');
                            $xhtml_link->addAttribute('hreflang', $language);
                            $xhtml_link->addAttribute('href', \System\Helper::addLanguageToURL($language, $url['loc']));
                        }
                    }
                }
            }
    ...
    public function updateRobots($pages, $disallow_pages)
    {
        ...
        foreach ($disallow_pages as $key => $disallow_page) {
            $sampleRobotsFile .= "\nDisallow: " . $disallow_pages[$key][0];
        };
